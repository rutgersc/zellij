...
impl PluginMap {
    ...
    ///
    /// Returns whether any quiesced instance was intercepting key presses, so the caller can
    /// renotify the screen-side state that mirrored those subscriptions.
    pub fn quiesce_client_instances(&mut self, client_id: ClientId) -> bool {
